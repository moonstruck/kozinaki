...
class AzureNodeDriver(NodeDriver):
    ...
    def create_node(self, ex_cloud_service_name=None, **kwargs):
        """Create Azure Virtual Machine
           ...
           Reference: http://msdn.microsoft.com/en-us/library/azure/jj157194.aspx
    ...
    def _to_node(self, data):
        ...
        ## Note: data.instance_enpoints is non existent in StoppedDeallocated state
        if data.instance_endpoints and len(data.instance_endpoints) >= 1:
    ...
    def _perform_put(self, path, body, response_type=None, async=False):
        ...
        request.path, request.query = self._update_request_uri_query(request)
        request.headers = self._update_management_header(request)
        new_body = '<?xml version="1.0" encoding="utf-8"?>' + request.body
...
class AzureXmlSerializer():
    ...
    def restart_role_operation_to_xml():
        ...
        doc = AzureXmlSerializer.doc_from_xml(
            ...
            xml)
        return ET.tostring(doc).replace("\n", "")

    @staticmethod
    def shutdown_role_operation_to_xml():

        '''
        Generating XML to shutdown role as per http://msdn.microsoft.com/en-us/library/azure/jj157195.aspx
        '''

        doc = AzureXmlSerializer.doc_from_xml('ShutdownRoleOperation', None)
        doc.append(AzureXmlSerializer.data_to_xml([('OperationType', 'ShutdownRoleOperation')], None))
        doc.append(AzureXmlSerializer.data_to_xml([('PostShutdownAction', 'StoppedDeallocated')], None))
    ...
    def add_role_to_xml(role_name, system_configuration_set,
        ...
        return ET.tostring(xml).replace("\n", "")
    ...
    def doc_from_xml(document_element_name, inner_xml=None):
        ...
        XHTML_NAMESPACE = "http://schemas.microsoft.com/windowsazure"
